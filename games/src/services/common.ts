 ...
 */
export function getRequiredUrl(gameName: string = '', userInfo = false) {

  const url = getWsUrl(gameName, userInfo);
  return url;
}
...
function getWsUrl(gameName = '', userInfo) {
  ...
  let url = `${wsBaseUrl}/${gameName}?userInfo=${encryptedUserInfo}`;
