...
import { CountdownStateChanged, HorseRaceGameStateChanged, Game, GameData, Input, Output, Plugin, SelectedHostStateChanged, ZodiacGameStateChanged } from "./Code";
...
export class PlayerManager implements Plugin {
    ...
    if(gameData.state.horseRace === GameState.Closed) {
      game.view(Player, Input, Output, UserData).each((entity, player, input, output, userData) => {
        if(hasValue(output.msg) && typeof output.msg === 'string'){
          ...
          newOutPut.horseStats = gameData.horseStats;
          output.msg = JSON.stringify(newOutPut);
        }else{
          output.insert("horseStats", gameData.horseStats);
        }
      });
    }
    ...
    gameData.games.forEach(gameName => {
      game.view(gameName === 'horseRace' ? HorseRaceGameStateChanged : null, Output, UserData).each((entity, stateChanged, output, userData) => {
        ...
        if (gameData.voidGame['horseRace']) {
            // Either 'dos' or 'zodiac' is true, set the corresponding game and message
            
            secondParam = {
                game: gameData.voidGame['horseRace'] && 'horseRace',
                message: 'Void Game!'
            };
        } else {
...
function broadcastWinners(game: Game) {
  ...
  gameData.games.forEach(gameName => {
    game.view(gameName === 'horseRace' ? HorseRaceGameStateChanged : null, UserData, Player, Prize, Output)
    .each((entity, stateChanged, userData, player, prize, output) => {
      if (gameData.state[gameName] === GameState.WinnerDeclared) {
        
        const winner = gameName === 'horseRace' && gameData.winnerOrders[gameName];
        ...
        if(player.game === gameName){
          ...
          const processBets = async () => {
            ...
            // Process each bet
            for (const [key, val] of player.slots.entries()) {
              const isWin = String(winner) === String(key);
              const processPromise = (async () => {

                if (isWin && gameName === prize.game) {
                  ...
                  const companyCommission = Number(val * (gameData.horseRaceCommission));
...
async function requestInit(game: Game, entity, gameData, msg, output, player, userData) {
    ...
    //incompleteGameRound ?  JSON.parse(incompleteGameRound.dataValues.slots) : 
      ...
      //latestBalance: isTesting === 'false' ? callbackResponse.data.credit : 0
		});
	}
}
...
async function processBet(userId, bet, odds, gameName, gameData, ball, winLossTransaction) {
  ...
  const companyCommission = Number(bet * (gameName === 'horseRace' && gameData.horseRaceCommission));
  await Transaction.new(w.id, gameData.gamesTableId[gameName], bet, "bet", odds, gameData.gameId[gameName]);
  const config = await Config.findOne({ where: { id: gameName === 'horseRace' && 3} });

  const overAllCommission = Number(bet * config.fee);
  await BetModel.new(gameData.gamesTableId[gameName], winLossTransaction.id, gameName === 'horseRace' && ball, gameData.gameId[gameName], companyCommission, overAllCommission);
}
...
const insertWinningBall = async (gameData: GameData, gameName) => {
	await WinningBall.new(gameData.gamesTableId[gameName], gameName === 'horseRace' && gameData.winnerOrders[gameName], gameName);
}