...
import { CountdownStateChanged, Game, GameData, Input, KaraKrusGameStateChanged, Output, Plugin, SelectedHostStateChanged, ZodiacGameStateChanged } from "./Code";
...
export class PlayerManager implements Plugin {
    ...
    gameData.games.forEach(gameName => {
      game.view(gameName === 'karakrus' ? KaraKrusGameStateChanged : null, Output).each((entity, stateChanged, output) => {
        ...
        if (gameData.voidGame['karakrus']) {
            // Either 'dos' or 'zodiac' is true, set the corresponding game and message
            
            secondParam = {
                game: gameData.voidGame['karakrus'] && 'karakrus',
                message: 'Void Game!'
            };
        } else {
...
function broadcastWinners(game: Game) {
  ...
  gameData.games.forEach(gameName => {
    game.view(gameName === 'karakrus' ? KaraKrusGameStateChanged : null, UserData, Player, Prize, Output)
    .each((entity, stateChanged, userData, player, prize, output) => {
      if (gameData.state[gameName] === GameState.WinnerDeclared) {
        
        const winner = gameName === 'karakrus' && gameData.winnerOrders[gameName];
            ...
            for (const [key, val] of player.slots.entries()) {
              const processPromise = (async () => {
                if (String(winner) === String(key) && gameName === prize.game) {
                  ...
                  const companyCommission = Number(val * (gameData.karakrusCommission));
  ...
  gameData.games.forEach(gameName => {
    game.view(gameName === 'karakrus' ? KaraKrusGameStateChanged : null, Output)
...
async function processBet(userId, bet, odds, gameName, gameData, ball, winLossTransaction) {
  ...
  const companyCommission = Number(bet * (gameName === 'karakrus' && gameData.karakrusCommission));
  await Transaction.new(w.id, gameData.gamesTableId[gameName], bet, "bet", odds, gameData.gameId[gameName]);
  const config = await Config.findOne({ where: { id: gameName === 'karakrus' && 3} });

  const overAllCommission = Number(bet * config.fee);
  await BetModel.new(gameData.gamesTableId[gameName], winLossTransaction.id, gameName === 'karakrus' && ball, gameData.gameId[gameName], companyCommission, overAllCommission);
}
