...
async function update(set, eventData) {
  ...
  if ((typeof meta === 'string' && meta.includes('gameId'))) {
    ...
    set({ gameId: gameId.gameId.boatRace });
  }
  ...
  if (hasValue(meta.slots)) {
    const slots = meta.slots.boatRace;
    if (hasValue(slots)) {  
  ...
  if ((hasValue(meta.allBets)) || (typeof meta === 'string' && meta.includes('allBets'))) {
    ...
    if (hasValue(arrayToMap(allBets.allBets.boatRace))) {
        set({ allBets: arrayToMap(allBets.allBets.boatRace)});
    }
  }

  if (hasValue(meta.odds)) {
    const odds = arrayToMap(meta.odds.boatRace);
    set({ odds: odds });
  }
  
  if ((typeof meta === 'string' && meta.includes('odds'))) {
    const newMeta = JSON.parse(meta);
    const newOdds = arrayToMap(newMeta.odds.boatRace);
    if(hasValue(newOdds)){
      set({ odds: newOdds });
    }
  }
  ...
  if (hasValue(meta.state) || (typeof meta === 'string' && meta.includes('state'))) {
    ...
    if (state.state.boatRace) {
        set({ gameState:state.state.boatRace});
    }
  }

  if (hasValue(meta.winners) || (typeof meta === 'string' && meta.includes('winners'))) {
    const winners = typeof meta ==='string' ? JSON.parse(meta) : meta;
    if(winners.winners.boatRace){
      set({ winners: meta.winners.boatRace});
      set({ winners: winners.winners.boatRace});
    }
  }
  ...
  if (hasValue(meta.countdown) || (typeof meta === 'string' && meta.includes('countdown'))) {
    ...
    set({ countdown: countdown.countdown.boatRace});
  }
  if (hasValue(meta.topPlayers)) {
    set({ topPlayers: meta.topPlayers.boatRace});
  }
...
function newGameState(set, meta) {
  ...
  const gameState = newMeta.state.boatRace;
