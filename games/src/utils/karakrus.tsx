...
export default function connect(set) {
  ...
  const url = getRequiredUrl('karakrus', userInfo);
  if (!hasValue(url)) {
...
async function update(set, eventData) {
  ...
  if ((typeof meta === 'string' && meta.includes('gameId'))) {
    ...
    set({ gameId: gameId.gameId.karakrus });
  }
  if ((hasValue(meta.gameId))) {
    set({ gameId: meta.gameId.karakrus });
  }
  ...
  if (hasValue(meta.slots)) {
    const slots = meta.slots.karakrus;
    if (hasValue(slots)) {  
  ...
  if ((hasValue(meta.allBets)) || (typeof meta === 'string' && meta.includes('allBets'))) {
    ...
    if (hasValue(arrayToMap(allBets.allBets.karakrus))) {
        set({ allBets: arrayToMap(allBets.allBets.karakrus)});
    }
  }

  if (hasValue(meta.odds)) {
    const odds = arrayToMap(meta.odds.karakrus);
    set({ odds: odds });
  }
  
  if ((typeof meta === 'string' && meta.includes('odds'))) {
    const newMeta = JSON.parse(meta);
    const newOdds = arrayToMap(newMeta.odds.karakrus);
    if(hasValue(newOdds)){
      set({ odds: newOdds });
    }
  }
  ...
  if (hasValue(meta.state) || (typeof meta === 'string' && meta.includes('state'))) {
    ...
    if (state.state.karakrus) {
        set({ gameState:state.state.karakrus});
    }
  }

  if (hasValue(meta.winners) || (typeof meta === 'string' && meta.includes('winners'))) {
    const winners = typeof meta ==='string' ? JSON.parse(meta) : meta;
    if(winners.winners.karakrus){
      set({ winners: meta.winners.karakrus});
      set({ winners: winners.winners.karakrus});
    }
  }
  ...
  if (hasValue(meta.countdown) || (typeof meta === 'string' && meta.includes('countdown'))) {
    ...
    set({ countdown: meta.countdown.karakrus});
    set({ countdown: countdown.countdown.karakrus});
  }
  if (hasValue(meta.topPlayers) || (typeof meta === 'string' && meta.includes('topPlayers'))) {
    ...
    if(hasValue(meta.topPlayers)){
      set({ topPlayers: meta.topPlayers.karakrus});
    }
    set({ topPlayers: topPlayers.topPlayers.karakrus});
  }
...
function newGameState(set, meta) {
  ...
  const gameState = newMeta.state.karakrus;
