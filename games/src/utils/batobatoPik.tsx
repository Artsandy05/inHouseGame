...
export default function connect(set) {
  ...
  const url = getRequiredUrl('bato-bato-pik', userInfo);
  if (!hasValue(url)) {
...
async function update(set, eventData) {
  ...
  if ((hasValue(meta.allBets)) || (typeof meta === 'string' && meta.includes('allBets'))) {
    ...
    if (hasValue(arrayToMap(allBets.allBets.bbp))) {
        set({ allBets: arrayToMap(allBets.allBets.bbp)});
    }
  }

  if (hasValue(meta.odds)) {
    const odds = arrayToMap(meta.odds.bbp);
    set({ odds: odds });
  }
  
  if ((typeof meta === 'string' && meta.includes('odds'))) {
    const newMeta = JSON.parse(meta);
    const newOdds = arrayToMap(newMeta.odds.bbp);
    if(hasValue(newOdds)){
      set({ odds: newOdds });
    }
  }
  ...
  if (hasValue(meta.winners) || (typeof meta === 'string' && meta.includes('winners'))) {
    ...
    if(winners.winners.bbp){
      set({ winners: meta.winners.bbp});
      set({ winners: winners.winners.bbp});
    }
  }
  ...
  if (hasValue(meta.countdown) || (typeof meta === 'string' && meta.includes('countdown'))) {
    ...
    set({ countdown: countdown.countdown.bbp});
  }
  if (hasValue(meta.topPlayers) || (typeof meta === 'string' && meta.includes('topPlayers'))) {
    ...
    if(hasValue(meta.topPlayers)){
      set({ topPlayers: meta.topPlayers.bbp});
    }
    set({ topPlayers: topPlayers.topPlayers.bbp});
  }
...
function newGameState(set, meta) {
  ...
  const gameState = newMeta.state.bbp;
