...
import { BoatRaceGameStateChanged, Game, GameData, Output, Plugin } from "./Code";
...
function update(game: Game) {
	...
	game.view(GameData).each((entity, g) => {
      ...
      gameData.games.forEach(gameName => {
        game.view(gameName === 'boatRace' && BoatRaceGameStateChanged, Player, UserData, Output).each((entity, state, player, userData, output) => { 
          // && player.slots.size > 0
