...
import { Game, GameData, KaraKrusGameStateChanged, Output, Plugin } from "./Code";
...
function update(game: Game) {
	...
	game.view(GameData).each((entity, g) => {
      ...
      gameData.games.forEach(gameName => {
        game.view(gameName === 'karakrus' && KaraKrusGameStateChanged, Player, UserData, Output).each((entity, state, player, userData, output) => { 
          // && player.slots.size > 0
