...
import { CountdownStateChanged, BoatRaceGameStateChanged, Game, GameData, Input, Output, Plugin, SelectedHostStateChanged, ZodiacGameStateChanged } from "./Code";
...
export class PlayerManager implements Plugin {
    ...
    if(gameData.state.boatRace === GameState.Closed) {
      game.view(Player, Input, Output, UserData).each((entity, player, input, output, userData) => {
        if(hasValue(output.msg) && typeof output.msg === 'string'){
          ...
          newOutPut.boatStats = gameData.boatStats;
          output.msg = JSON.stringify(newOutPut);
        }else{
          output.insert("boatStats", gameData.boatStats);
        }
      });
    }
    ...
    gameData.games.forEach(gameName => {
      game.view(gameName === 'boatRace' ? BoatRaceGameStateChanged : null, Output).each((entity, stateChanged, output) => {
        ...
        if (gameData.voidGame['boatRace']) {
            // Either 'dos' or 'zodiac' is true, set the corresponding game and message
            
            secondParam = {
                game: gameData.voidGame['boatRace'] && 'boatRace',
                message: 'Void Game!'
            };
        } else {
...
function broadcastWinners(game: Game) {
  ...
  gameData.games.forEach(gameName => {
    game.view(gameName === 'boatRace' ? BoatRaceGameStateChanged : null, UserData, Player, Prize, Output)
  ...
  gameData.games.forEach(gameName => {
    game.view(gameName === 'boatRace' ? BoatRaceGameStateChanged : null, Output)
...
async function processBet(userId, bet, odds, gameName, gameData, ball, winLossTransaction) {
  ...
  const companyCommission = Number(bet * (gameName === 'boatRace' && gameData.boatRaceCommission));
  await Transaction.new(w.id, gameData.gamesTableId[gameName], bet, "bet", odds, gameData.gameId[gameName]);
  const config = await Config.findOne({ where: { id: gameName === 'boatRace' && 3} });

  const overAllCommission = Number(bet * config.fee);
  await BetModel.new(gameData.gamesTableId[gameName], winLossTransaction.id, gameName === 'boatRace' && ball, gameData.gameId[gameName], companyCommission, overAllCommission);
}
...
const insertWinningBall = async (gameData: GameData, gameName) => {
	await WinningBall.new(gameData.gamesTableId[gameName], gameName === 'boatRace' && gameData.winnerOrders[gameName], gameName);
}